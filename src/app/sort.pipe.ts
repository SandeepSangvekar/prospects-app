 ...
 */
 ...
 export class SortPipe implements PipeTransform {
   ...
   private sortArray(value: any[], expression?: any, reverse?: boolean): any[] {
     ...
     if (isDeepLink) {
       expression = this.OrderByPipe.parseExpression(expression);
     }
     ...
     const array: any[] = value.sort((a: any, b: any): number => {
       ...
       return this.OrderByPipe.getValue(a, expression) > this.OrderByPipe.getValue(b, expression) ? 1 : -1;
     });
   ...
   private transformObject(value: any | any[], expression?: any, reverse?: boolean): any {
     const parsedExpression = this.OrderByPipe.parseExpression(expression);
     let lastPredicate = parsedExpression.pop();
     let oldValue = this.OrderByPipe.getValue(value, parsedExpression);
 
     if (!(oldValue instanceof Array)) {
       parsedExpression.push(lastPredicate);
       lastPredicate = null;
       oldValue = this.OrderByPipe.getValue(value, parsedExpression);
     }
     ...
     this.OrderByPipe.setValue(value, newValue, parsedExpression);
     return value;
   }
